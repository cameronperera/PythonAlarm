...
def checkIfFileExist(fileCheck):
...
def getStoredPunches():
    fileCheck = checkIfFileExist('punchTimes.txt')
    if fileCheck == True:
...
def start():
    ...
    while whileVar == True:
        ...
        if fileCheck == 'new':
            ...
            checkFile = checkIfFileExist('punchTimes.txt')
            if checkFile == True:
                punch = getStoredPunches()
                whileVar = False
                return punch
            else:
                print('No stored punch times, please create new punch times.')
        else:
...
while True:
    ...
    if dayNumber == saturday:
        time.sleep(sleep2days)
    else:
        presentTime = currentTimeArray()
        print('The present time is ' + str(presentTime))
        nextPunch = selectPunchTime(alarm)
        print('The next selected punch time is ' + str(nextPunch))
        timeDifference = substractPunchFromCurrent(presentTime, nextPunch)
        sleepSeconds = secondsToSleep(timeDifference)
        print('Seconds that the program will sleep is ' + str(sleepSeconds))
        print('')
        putProgramToSleep(sleepSeconds)
        alarmWindow()
